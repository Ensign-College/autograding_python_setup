...
class TestRunSingleTest(unittest.TestCase):
    @patch('builtins.open', new_callable=mock_open, read_data='{"tests": [{"name": "test1", "points": 10}]}')
    def test_run_single_test_online(self, mock_file):
        ...
        with patch('central_setup.central_setup.check_internet_connection', return_value=True):
    ...
    def test_run_single_test_offline(self, mock_file):
        ...
        with patch('central_setup.central_setup.check_internet_connection', return_value=False):
            # Should not raise an assertion error
            run_single_test("test1", "test1", "expected feedback", pre_test_setup)
...
class TestGetGithubToken(unittest.TestCase):
    ...
    def test_get_github_token_windows(self, mock_system):
        ...
        try:
            ...
            with patch('win32cred.CredRead', return_value={'CredentialBlob': b'mock-token'}):
                token = get_github_token()
                self.assertEqual(token, 'mock-token')
        except ImportError:
...
class TestGetGithubUsernameAndSlug(unittest.TestCase):
    ...
    def test_get_github_username_and_slug_success(self, mock_basename, mock_requests_get):
    ...
    def test_get_github_username_and_slug_invalid_directory(self, mock_basename, mock_requests_get):
        # Mock successful GitHub API response
        mock_response = MagicMock()
        mock_response.ok = True
        mock_response.json.return_value = {'login': 'username'}
...
class TestRunProgram(unittest.TestCase):
    ...
    @patch('subprocess.run', side_effect=subprocess.CalledProcessError(1, [], stderr="Error message"))
...
class TestExecuteLogic(unittest.TestCase):
    ...
    @patch('central_setup.central_setup.get_github_username_and_slug', return_value=('username', 'slug'))
    @patch('central_setup.central_setup.check_internet_connection', return_value=True)
    @patch('requests.post')
    def test_execute_logic_with_env_var(self, mock_post, mock_check_internet, mock_get_username, mock_get_token):
        ...
        with patch.dict(os.environ, {'AUTOGRADING_BASE_URL': test_url}):
            ...
            self.assertEqual(args[0], test_url)  # First positional arg should be the URL
    ...
    @patch('central_setup.central_setup.check_internet_connection', return_value=True)
    @patch('requests.post')
    def test_execute_logic_default_url(self, mock_post, mock_check_internet, mock_get_username, mock_get_token):
        # Mock response for post request
        mock_response = MagicMock()
        mock_response.json.return_value = {
            'tests': [{'name': 'test1', 'pointsAwarded': 10, 'feedback': 'Good job'}],
        ...
        with patch('os.environ.get') as mock_get_env:
            ...
            mock_get_env.return_value = 'https://autograding-api-next.vercel.app/api/autograde'
            ...
            mock_get_env.assert_called_with('AUTOGRADING_BASE_URL', 'https://autograding-api-next.vercel.app/api/autograde')
            
            # Verify URL used in the request
            mock_post.assert_called_once()
            args, kwargs = mock_post.call_args
            self.assertEqual(args[0], 'https://autograding-api-next.vercel.app/api/autograde')
            
    @patch('central_setup.central_setup.get_github_token', return_value='mock-token')
    @patch('central_setup.central_setup.get_github_username_and_slug', return_value=('username', 'slug'))
    @patch('central_setup.central_setup.check_internet_connection', return_value=False)
    @patch('requests.post')
    def test_execute_logic_no_internet(self, mock_post, mock_check_internet, mock_get_username, mock_get_token):
        # Mock response for post request
        mock_response = MagicMock()
        ...
        with patch('builtins.print') as mock_print:
            ...
            mock_print.assert_any_call("No active internet connection or API response. Run the test again with an active internet connection and a working API to receive more user-friendly feedback.")
            
    @patch('central_setup.central_setup.get_github_token', return_value=None)
    @patch('central_setup.central_setup.get_github_username_and_slug', return_value=(None, None))
    @patch('central_setup.central_setup.check_internet_connection', return_value=True)
    @patch('requests.post')
    def test_execute_logic_no_token(self, mock_post, mock_check_internet, mock_get_username, mock_get_token):
        # Mock response for post request
        mock_response = MagicMock()
        mock_response.json.return_value = {
            'tests': [{'name': 'test1', 'pointsAwarded': 10, 'feedback': 'Good job'}],
    ...
    @patch('central_setup.central_setup.check_internet_connection', return_value=True)
    @patch('requests.post')
    def test_execute_logic_request_exception(self, mock_post, mock_check_internet, mock_get_username, mock_get_token):
