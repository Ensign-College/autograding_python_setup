...
def run_single_test(test_name, test_point_key, offline_feedback, pre_test_setup):
    test_outputs, test_points_awarded, test_feedback, test_response_data = (
        pre_test_setup(test_name=test_name)
    )
    ...
    test_config = next(
        (
            test
            for test in autograding_config["tests"]
            if test["name"] == test_point_key
        ),
        None,
    )
    ...
    if check_internet_connection():
        assert (
            test_points_awarded.get(test_point_key, 0) == expected_points
        ), test_feedback
    else:
    ...
    if system == "Linux":
        ...
        try:
            ...
            creds = win32cred.CredRead(
                "git:https://github.com", win32cred.CRED_TYPE_GENERIC
            )
...
def get_github_username_and_slug(github_token):
    ...
    if github_token:
        try:
            ...
            if github_response.ok:
                ...
                print(
                    f"Unable to verify GitHub token: {github_response.status_code} {github_response.reason}"
                )
    ...
    if gitHubUserId:
        ...
        if gitHubUserId in current_directory:
            ...
            print(f"Unable to detect slug in {current_directory}.")
    return None, None
    ...
    try:
        ...
        result = subprocess.run(
            [
                "python3",
                program_name,
            ],  # Adjust this command if using a different Python version or path
            input=input_data,
            text=True,
            capture_output=True,
...
def execute_logic(
    ...
    autograding_base_url = os.environ.get(
        "AUTOGRADING_BASE_URL", "https://autograding-api-next.vercel.app/api/autograde"
    )

    # Send the POST request
    response = requests.post(autograding_base_url, json=data, headers=headers)
    ...
    test_points_awarded = {
        test["name"]: test["pointsAwarded"] for test in test_response_data["tests"]
    }
    test_feedback = ""
            ...
            test_feedback = (
                ...
                + "\n".join(
                    [
                        ...
                        for feedback in test_response_data["specificCodeFeedback"][
                            "code"
                        ]
                    ]
                )
